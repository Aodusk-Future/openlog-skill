...
> A lightweight per-module development journal for [Claude Code](https://claude.com/claude-code) and [Codex CLI](https://github.com/openai/codex) — keep architecture, dev log, and current-stage notes for every module of a large project, so any future session or agent can pick up the work in under 1k tokens.
>
> 一个轻量级的"按模块开发档案"工具,同时支持 [Claude Code](https://claude.com/claude-code) 和 [Codex CLI](https://github.com/openai/codex)——为大型项目的每个模块沉淀架构说明、开发日志、当前断点,让未来任何会话 / agent 都能在 1k token 以内接手工作。
...
Works with both [Claude Code](https://claude.com/claude-code) and [Codex CLI](https://github.com/openai/codex) — they use the same SKILL.md format.

```bash
# For Claude Code
git clone https://github.com/Aodusk-Future/openlog-skill ~/.claude/skills/openlog

# For Codex CLI (identical workflow, same trigger commands)
...
…or just say something like "let's work on the Combat module" / "start the Combat module" — the agent will recognise the trigger and run the OpenLog workflow.
...
同时支持 [Claude Code](https://claude.com/claude-code) 和 [Codex CLI](https://github.com/openai/codex)——两者用相同的 SKILL.md 格式。

```bash
# Claude Code
git clone https://github.com/Aodusk-Future/openlog-skill ~/.claude/skills/openlog

# Codex CLI（工作流完全一致,触发命令也相同）
git clone https://github.com/Aodusk-Future/openlog-skill ~/.codex/skills/openlog
```
...
…或者直接说"开始开发 Combat 模块" / "接手战斗模块"等,agent 会识别触发条件并走 OpenLog 工作流。
